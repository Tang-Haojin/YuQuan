...
case class RVM()(implicit val p: Parameters) extends CPUParams {
  ...
  def MULW   = if(xlen==64) BitPat("b0000001_?????_?????_000_?????_0111011") else RVI().ERR
  def DIVW   = if(xlen==64) BitPat("b0000001_?????_?????_100_?????_0111011") else RVI().ERR
  def DIVUW  = if(xlen==64) BitPat("b0000001_?????_?????_101_?????_0111011") else RVI().ERR
  def REMW   = if(xlen==64) BitPat("b0000001_?????_?????_110_?????_0111011") else RVI().ERR
  def REMUW  = if(xlen==64) BitPat("b0000001_?????_?????_111_?????_0111011") else RVI().ERR
