package cpu.instruction

import chisel3._
import chisel3.util._
import chipsalliance.rocketchip.config._

import cpu.pipeline.NumTypes._
import cpu.component.Operators._
import cpu.pipeline.InstrTypes._
import cpu.pipeline.ExecSpecials._
import cpu._

case class RVC()(implicit val p: Parameters) extends CPUParams {
  // quadrant 0
  private def C_ADDI4SPN = BitPat("b????????????????_000_???_???_??_???_00") // when nzuimm = 0, raise an invalid instruction exception
  private def C_LW       = BitPat("b????????????????_010_???_???_??_???_00")
  private def C_LD       = BitPat("b????????????????_011_???_???_??_???_00") // RV64/128
  private def C_SW       = BitPat("b????????????????_110_???_???_??_???_00")
  private def C_SD       = BitPat("b????????????????_111_???_???_??_???_00") // RV64/128

  // quadrant 1
  private def C_ADDI     = BitPat("b????????????????_000_?_?????_?????_01")
  private def C_JAL      = BitPat("b????????????????_001_?_?????_?????_01") // RV32
  private def C_ADDIW    = BitPat("b????????????????_001_?_?????_?????_01") // RV64/128
  private def C_LI       = BitPat("b????????????????_010_?_?????_?????_01")
  private def C_LUI16SP  = BitPat("b????????????????_011_?_?????_?????_01")
  private def C_SRLI     =
            if(xlen!=32)   BitPat("b????????????????_100_?_00???_?????_01")
            else           BitPat("b????????????????_100_0_00???_?????_01")
  private def C_SRAI     =
            if(xlen!=32)   BitPat("b????????????????_100_?_01???_?????_01")
            else           BitPat("b????????????????_100_0_01???_?????_01")
  private def C_ANDI     = BitPat("b????????????????_100_?_10???_?????_01")
  private def C_SUB      = BitPat("b????????????????_100_0_11???_00???_01")
  private def C_XOR      = BitPat("b????????????????_100_0_11???_01???_01")
  private def C_OR       = BitPat("b????????????????_100_0_11???_10???_01")
  private def C_AND      = BitPat("b????????????????_100_0_11???_11???_01")
  private def C_SUBW     = BitPat("b????????????????_100_1_11???_00???_01") // RV64/128
  private def C_ADDW     = BitPat("b????????????????_100_1_11???_01???_01") // RV64/128
  private def C_J        = BitPat("b????????????????_101_?_?????_?????_01")
  private def C_BEQZ     = BitPat("b????????????????_110_?_?????_?????_01")
  private def C_BNEZ     = BitPat("b????????????????_111_?_?????_?????_01")

  // quadrant 2
  private def C_SLLI     =
            if(xlen!=32)   BitPat("b????????????????_000_?_?????_?????_10")
            else           BitPat("b????????????????_000_0_?????_?????_10")
  private def C_LWSP     = BitPat("b????????????????_010_?_?????_?????_10") // rd != 0
  private def C_LDSP     = BitPat("b????????????????_011_?_?????_?????_10") // RV64/128, rd != 0
  private def C_JRMV     = BitPat("b????????????????_100_0_?????_?????_10")
  private def C_JLREBKAD = BitPat("b????????????????_100_1_?????_?????_10")
  private def C_SWSP     = BitPat("b????????????????_110_?_?????_?????_10")
  private def C_SDSP     = BitPat("b????????????????_111_?_?????_?????_10") // RV64/128

  val table = List(
    //                | Type  |num1 |num2 |num3 |num4 |op1_2| WB |Special|
    C_ADDI4SPN -> List(caddi4 , x2  , imm , non , non , add , 1.B, norm  ), // quadrant 0
    C_LW       -> List(cldst  , non , non , rs1p, imm , nop , 1.B, ld    ),
    C_SW       -> List(cldst  , rs2p, non , rs1p, imm , add , 0.B, st    )) ++ (if (xlen != 32) List(
    C_LD       -> List(cldst  , non , non , rs1p, imm , nop , 1.B, ld    ),
    C_SD       -> List(cldst  , rs2p, non , rs1p, imm , add , 0.B, st    )) else Nil) ++ List( // quadrant 1
    C_ADDI     -> List(c540   , rd1c, imm , non , non , add , 1.B, norm  ),
    C_LI       -> List(c540   , non , imm , non , non , add , 1.B, norm  ),
    C_LUI16SP  -> List(clui   , rd1c, imm , non , non , add , 1.B, norm  ),
    C_SRLI     -> List(c540   , rd1p, imm , non , non , srl , 1.B, norm  ),
    C_SRAI     -> List(c540   , rd1p, imm , non , non , sra , 1.B, norm  ),
    C_ANDI     -> List(c540   , rd1p, imm , non , non , and , 1.B, norm  ),
    C_SUB      -> List(cni    , rd1p, rs2p, non , non , sub , 1.B, norm  ),
    C_XOR      -> List(cni    , rd1p, rs2p, non , non , xor , 1.B, norm  ),
    C_OR       -> List(cni    , rd1p, rs2p, non , non , or  , 1.B, norm  ),
    C_AND      -> List(cni    , rd1p, rs2p, non , non , and , 1.B, norm  ),
    C_J        -> List(cj     , pc  , non , imm , non , add , 0.B, norm  ),
    C_BEQZ     -> List(cb     , rs1p, non , imm , non , nop , 0.B, norm  ),
    C_BNEZ     -> List(cb     , rs1p, non , imm , non , nop , 0.B, norm  )) ++ (if (xlen == 32) List(
    C_JAL      -> List(cj     , pc  , two , imm , non , add , 1.B, norm  )) else List(
    C_ADDIW    -> List(c540   , rd1c, imm , non , non , add , 1.B, word  ),
    C_SUBW     -> List(cni    , rd1p, rs2p, non , non , sub , 1.B, word  ),
    C_ADDW     -> List(cni    , rd1p, rs2p, non , non , add , 1.B, word  ))) ++ List( // quadrant 2
    C_SLLI     -> List(c540   , rd1c, imm , non , non , sll , 1.B, norm  ),
    C_LWSP     -> List(clsp   , non , non , x2  , imm , nop , 1.B, ld    ),
    C_JRMV     -> List(cni    , non , rs2c, non , rs1c, add , 1.B, norm  ),
    C_JLREBKAD -> List(cni    , rd1c, rs2c, non , rs1c, add , 1.B, norm  ),
    C_SWSP     -> List(cssp   , rs2c, non , x2  , imm , add , 0.B, st    )) ++ (if (xlen != 32) List(
    C_LDSP     -> List(clsp   , non , non , x2  , imm , nop , 1.B, ld    ),
    C_SDSP     -> List(cssp   , rs2c, non , x2  , imm , add , 0.B, st    )) else Nil)
}
