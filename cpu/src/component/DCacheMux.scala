...
class DCacheMux(implicit p: Parameters) extends YQModule {
  ...
  io.cpuIO.cpuResult := Mux(current === cpu, io.dcacheIO.cpuResult, 0.U.asTypeOf(new CpuResult()))
  io.dmaIO.cpuResult := Mux(current =/= cpu, io.dcacheIO.cpuResult, 0.U.asTypeOf(new CpuResult()))
