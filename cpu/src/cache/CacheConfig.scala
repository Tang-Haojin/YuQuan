...
object CacheConfig {
  val f: (View, View, View) => PartialFunction[Any,Any] = (site, here, up) => {
    case BUSLEN        => up(XLEN)
    case CACHE_SIZE    => site(GEN_NAME) match { case "ysyx" => 4 * 1024; case "zmb" => 32 * 1024; case "lxb" => 16 * 1024 } // in byte
    case ASSOCIATIVITY => 4
    case BLOCK_SIZE    => site(GEN_NAME) match { case "ysyx" => 16; case "zmb" => 128; case "lxb" => 64 } // in byte
