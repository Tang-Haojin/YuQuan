...
class AXI_BUNDLE_A(identifier: String)(implicit val p: Parameters, valName: ValName) extends Bundle with UtilsParams {
  val id     = UInt(idlen.W)
...
class AXI_BUNDLE_W(implicit p: Parameters, valName: ValName) extends ReadyValidIO(new Bundle {
...
class AXI_BUNDLE_B(implicit p: Parameters, valName: ValName) extends ReadyValidIO(new Bundle {
...
class AXI_BUNDLE_R(implicit p: Parameters, valName: ValName) extends ReadyValidIO(new Bundle {
  val id   = UInt(p(IDLEN).W)
  val data = UInt(p(XLEN).W)
  val resp = UInt(2.W)
  val last = Bool()
